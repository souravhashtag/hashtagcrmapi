  ...
  static getAttendanceByDateRange = async (req, res) => {
    try {
      ...
      let attendanceRecords = await Attendance.find({
        ...
        userId: req.params.employeeId !== 'all' ? req.params.employeeId : { $ne: null }
      })
        .populate({
          path: 'userId',
          select: '_id firstName lastName email',
      ...
      console.log("attendanceRecords===>", attendanceRecords.length);
      // Get employee info
...
// IF user forgets to logout, this API checks if there is any clock-in record without clock-out for more than 2 hours
  static checkMissedClockOut = async (req, res) => {
    try {
      const record = await Attendance.findOne({
        userId: req.user.id,
        clockOut: null,
      ...
      if (!record) {
        return res.json({ hasMissedClockOut: false });
      }

      const now = moment();
      const diffHours = now.diff(moment(record.clockIn), 'hours');
      ...
      res.json({ hasMissedClockOut: false });
    } catch (err) {
      res.status(500).json({ message: 'Error checking missed clock-out', error: err.message });
    }
  };
  static manualClockOut = async (req, res) => {
    try {
      const { recordId, manualClockOutTime, reason } = req.body;

      const record = await Attendance.findById(recordId);
      if (!record) return res.status(404).json({ message: 'Record not found' });

      record.clockOut = new Date(manualClockOutTime);
      record.manualClockOutReason = reason;
      record.calculateTotalHours();
      await record.save();

      res.json({ status: 200, message: 'Manual clock-out updated', data: record });
    } catch (err) {
      res.status(500).json({ message: 'Error updating manual clock-out', error: err.message });
    }
  };
